import colorful from "../assets/colorful.jpeg";
import trump from "../assets/people/trump.jpeg";
import abe from "../assets/people/abe.jpg";
import johnson from "../assets/people/johnson.jpeg";

import timelineImg1 from "../assets/img/timeline/web/12-years-education_web_bg.jpg";
import timelineImg2 from "../assets/img/timeline/web/12-years-education_web_pic1.jpg";
import timelineImg3 from "../assets/img/timeline/web/12-years-education_web_pic2.jpg";
import timelineImg4 from "../assets/img/timeline/web/12-years-education_web_pic3.jpg";
import timelineImg5_1 from "../assets/img/timeline/web/12-years-education_web_chart.svg";
import timelineImg5_2 from "../assets/img/timeline/web/12-years-education_web_pic4.jpg";
import timelineImg6 from "../assets/img/timeline/web/12-years-education_web_pic5.jpg";
import timelineImg7 from "../assets/img/timeline/web/12-years-education_web_pic6.jpg";
import timelineImg9 from "../assets/img/timeline/web/12-years-education_web_pic7.jpg";
import timelineImg10 from "../assets/img/timeline/web/12-years-education_web_pic8.jpg";
import timelineImg11 from "../assets/img/timeline/web/12-years-education_web_pic9.jpg";

import timelimePadImgPic0 from "../assets/img/timeline/pad/12-years-education_pad_bg.jpg";
import timelimePadImgPic1 from "../assets/img/timeline/pad/12-years-education_pad_pic1.jpg";
import timelimePadImgPic2 from "../assets/img/timeline/pad/12-years-education_pad_pic2.jpg";
import timelimePadImgPic3 from "../assets/img/timeline/pad/12-years-education_pad_pic3.jpg";
import timelimePadImgPic4_0 from "../assets/img/timeline/pad/12-years-education_pad_chart.svg";
import timelimePadImgPic4 from "../assets/img/timeline/pad/12-years-education_pad_pic4.jpg";
import timelimePadImgPic5 from "../assets/img/timeline/pad/12-years-education_pad_pic5.jpg";
import timelimePadImgPic6 from "../assets/img/timeline/pad/12-years-education_pad_pic6.jpg";
import timelimePadImgPic7 from "../assets/img/timeline/pad/12-years-education_pad_pic7.jpg";
import timelimePadImgPic8 from "../assets/img/timeline/pad/12-years-education_pad_pic8.jpg";
import timelimePadImgPic9 from "../assets/img/timeline/pad/12-years-education_pad_pic9.jpg";

import timelimeMobImgPic0 from "../assets/img/timeline/mob/12-years-education_mob_bg.jpg";
import timelimeMobImgPic1 from "../assets/img/timeline/mob/12-years-education_mob_pic1.jpg";
import timelimeMobImgPic2 from "../assets/img/timeline/mob/12-years-education_mob_pic2.jpg";
import timelimeMobImgPic3 from "../assets/img/timeline/mob/12-years-education_mob_pic3.jpg";
import timelimeMobImgPic4_0 from "../assets/img/timeline/mob/12-years-education_mob_chart.svg";
import timelimeMobImgPic4 from "../assets/img/timeline/mob/12-years-education_mob_pic4.jpg";
import timelimeMobImgPic5 from "../assets/img/timeline/mob/12-years-education_mob_pic5.jpg";
import timelimeMobImgPic6 from "../assets/img/timeline/mob/12-years-education_mob_pic6.jpg";
import timelimeMobImgPic7 from "../assets/img/timeline/mob/12-years-education_mob_pic7.jpg";
import timelimeMobImgPic8 from "../assets/img/timeline/mob/12-years-education_mob_pic8.jpg";
import timelimeMobImgPic9 from "../assets/img/timeline/mob/12-years-education_mob_pic9.jpg";

import otherProjectsWebImg1 from "../assets/img/timeline/relate/12-years-education_r1_web.jpg";
import otherProjectsWebImg3 from "../assets/img/timeline/relate/12-years-education_r3_web.jpg";
import otherProjectsWebImg4 from "../assets/img/timeline/relate/12-years-education_r4_web.jpg";
import otherProjectsWebImg5 from "../assets/img/timeline/relate/12-years-education_r5_web.jpg";
import otherProjectsWebImg6 from "../assets/img/timeline/relate/12-years-education_r6_web.jpg";
import otherProjectsMobImg1 from "../assets/img/timeline/relate/12-years-education_r1_mob.jpg";
import otherProjectsMobImg3 from "../assets/img/timeline/relate/12-years-education_r3_mob.jpg";
import otherProjectsMobImg4 from "../assets/img/timeline/relate/12-years-education_r4_mob.jpg";
import otherProjectsMobImg5 from "../assets/img/timeline/relate/12-years-education_r5_mob.jpg";
import otherProjectsMobImg6 from "../assets/img/timeline/relate/12-years-education_r6_mob.jpg";
import videoImg1 from "../assets/img/timeline/12-years-education_v1.jpg";
import videoImg2 from "../assets/img/timeline/12-years-education_v2.jpg";
import videoImg3 from "../assets/img/timeline/12-years-education_v3.jpg";
import videoImg4 from "../assets/img/timeline/12-years-education_v4.jpg";

import pressLikeImg1 from "../assets/img/poll/poll_good_white.svg";
import pressLikeImg2 from "../assets/img/poll/poll_good_blue.svg";

const content = {
  headerLink: [
    { text: "揭露課綱五大亂象", link: "../problem" },
    { text: "課綱上路周年大調查", link: "../poll" },
    { text: "台灣教育關鍵數字", link: "../problem" },
    { text: "台灣教改為何總失敗", link: "../" },
    { text: "升學主義悲歌", link: "../story" },
    { text: "關於108課綱 我想說......", link: "..collect/" },
    { text: "更多課綱相關報導", link: "/" },
  ],
  timelineImgs: {
    part1: timelineImg1,
    part2: timelineImg2,
    part3: timelineImg3,
    part4: timelineImg4,
    part5: [timelineImg5_1, timelineImg5_2],
    part6: timelineImg6,
    part7: timelineImg7,
    part9: timelineImg9,
    part10: timelineImg10,
    part11: timelineImg11,
  },
  timelineMobImgs: {
    part0: { pad: timelimePadImgPic0, mob: timelimeMobImgPic0 },
    part1: { pad: timelimePadImgPic1, mob: timelimeMobImgPic1 },
    part2: { pad: timelimePadImgPic2, mob: timelimeMobImgPic2 },
    part3: { pad: timelimePadImgPic3, mob: timelimeMobImgPic3 },
    part4_0: { pad: timelimePadImgPic4_0, mob: timelimeMobImgPic4_0 },
    part4: { pad: timelimePadImgPic4, mob: timelimeMobImgPic4 },
    part5: { pad: timelimePadImgPic5, mob: timelimeMobImgPic5 },
    part6: { pad: timelimePadImgPic6, mob: timelimeMobImgPic6 },
    part8: { pad: timelimePadImgPic7, mob: timelimeMobImgPic7 },
    part9: { pad: timelimePadImgPic8, mob: timelimeMobImgPic8 },
    part10: { pad: timelimePadImgPic9, mob: timelimeMobImgPic9 },
  },
  timelineVideo: [
    {
      img: videoImg1,
      name: "劉 安 婷",
      id: videoImg1,
      description: "TEACH FOR TAIWAN為台灣而教",
      text: "聯合報帶讀者進入教改時間洪流，從一個個教育政策",
      videoId: "MGGd7Mjs6J4",
    },
    {
      img: videoImg2,
      name: "嚴 長 壽",
      id: videoImg2,
      description: "TEACH FOR TAIWAN為台灣而教",
      text: "聯合報帶讀者進入教改時間洪流，從一個個教育政策",
      videoId: "roL7pH07iqE",
    },
    {
      img: videoImg3,
      name: "我 是 誰",
      id: videoImg3,
      description: "TEACH FOR TAIWAN為台灣而教",
      text: "聯合報帶讀者進入教改時間洪流，從一個個教育政策",
      videoId: "EcYX7DtGFLk",
    },
    {
      img: videoImg4,
      name: "管 中 閔",
      id: videoImg4,
      description: "TEACH FOR TAIWAN為台灣而教",
      text: "聯合報帶讀者進入教改時間洪流，從一個個教育政策",
      videoId: "lyBrpwTIiAU",
    },
  ],
  otherProjects: [
    {
      title: "無效預算？揭露108課綱五大亂象",
      img: { web: otherProjectsWebImg1, mob: otherProjectsMobImg1 },
      id: `${otherProjectsWebImg1}${otherProjectsMobImg1}`,
      link: "/",
    },
    {
      title: "課綱上路周年大調查",
      img: { web: otherProjectsWebImg4, mob: otherProjectsMobImg4 },
      id: "otherProjectsWebImg2otherProjectsMobImg2",
      link: "/",
    },
    {
      title: "台灣教育關鍵數字",
      img: { web: otherProjectsWebImg3, mob: otherProjectsMobImg3 },
      id: `${otherProjectsWebImg3}${otherProjectsMobImg3}`,
      link: "/",
    },
    {
      title: "台灣教改為何總失敗",
      img: { web: otherProjectsWebImg4, mob: otherProjectsMobImg4 },
      id: `${otherProjectsWebImg4}${otherProjectsMobImg4}`,
      link: "/",
    },
    {
      title: "升學主義悲歌：19歲少女的告白",
      img: { web: otherProjectsWebImg5, mob: otherProjectsMobImg5 },
      id: `${otherProjectsWebImg5}${otherProjectsMobImg5}`,
      link: "/",
    },
    {
      title: "關於108課綱 我想說......",
      img: { web: otherProjectsWebImg6, mob: otherProjectsMobImg6 },
      id: `${otherProjectsWebImg6}${otherProjectsMobImg6}`,
      link: "/",
    },
  ],
  marketing: [
    { img: "", text: "黎孔平 X 徐沛然", date: "08.24", id: "黎孔平X徐沛然1" },
    { img: "", text: "黎孔平 X 徐沛然", date: "08.24", id: "黎孔平X徐沛然2" },
  ],

  page2Articles: [
    { title: "1testtesttesttesttestest", id: "gfdgdfgs", height: "500px" },
    {
      title: "2testtesttesttesttestest",
      id: "gfdgdfgssdfsfs",
      height: "300px",
    },
    {
      title: "34testtesttesttesttestest",
      id: "gfdgdfgssfsvxv",
      height: "800px",
    },
    {
      title: "5testtesttesttesttestest",
      id: "gfdgdfgsxcvsgsf",
      height: "600px",
    },
    {
      title: "6testtesttesttesttestest",
      id: "gfdgdfgfsffewbjs",
      height: "200px",
    },
    {
      title: "7testtesttesttesttestest",
      id: "gfdgdfgsjgjyjhf",
      height: "700px",
    },
    {
      title: "8testtesttesttesttestest",
      id: "gfdgdfgshrhtr54",
      height: "400px",
    },
  ],
  theirVoice: [
    { img: trump, id: "trump", description: "我想贏得選舉" },
    { img: abe, id: "abe", description: "奧運能辦了嗎" },
    { img: johnson, id: "johnson", description: "你說我頭髮很亂？" },
  ],
  progress: { img: colorful },
  twinkleArticle: [
    "升學壓力鬆綁才有助改革",
    "與現實有差距",
    "素養導向教學仍太模糊",
    "教學現場難落實",
    "期待改變家長對第一志願的迷思",
    "最難翻轉的是老師數十年的舊思維",
    "家長接受度不高",
    "要給老師更多支持",
    "教學現場還停留在過去講述法",
    "立意良好，但與現實有差距",
    "升學壓力的緊箍咒要調整",
    "應該要建立教師淘汰機制",
    "匆促上路配套不足",
    "感覺像趕鴨子上架",
    "且戰且走",
    "學生生活經驗差異大",
    "難落實素養教學",
    "家長傳統升學觀念難撼動",
    "國中升高中的免試入學制度已形同虛設",
    "恐步上九年一貫後塵，失敗收場",
    "推動108課綱同時，能保持或提升學生學力",
    "教育是在實踐中完成的",
    "教師能否改變是新課綱能成功的關鍵",
    "請不要浪費錢或浪費時間做你們覺得對的事情",
    "教職生涯20年，已經歷三次教改，社會消化的了嗎？",
    "若學生的升學進路沒有變化，對學生的改變並不大",
    "匆忙上路，沒有考慮到學校的師資、設備是否準備好",
    "素養要有最高層級的培訓單位，否則瞎子摸象、徬徨失措",
    "希望教育部不要朝令夕改",
    "希望大學認真看待高中的努力",
    "不充實員額，嚴重限縮新課綱發展",
    "安學校教師的心才能落實108課綱",
    "需家長觀念轉換",
    "讓會考回歸學力檢測",
    "大家共同努力",
    "要先改變升學主義及明星學校",
    "鄉鎮學校的壓力在於資源不足",
  ],
  pressLikeImgs: {
    unlike: pressLikeImg1,
    like: pressLikeImg2,
  },
};

export default content;
